...
// };
"use client";
// 参考：https://engineer-blog.ajike.co.jp/web-camera-change/
...
const Camera = () => {
  ...
  const takePhoto = () => {
    if(refVideo.current && refCanvas.current) {
      ...
      if (context) {
        ...
        // 画像データを取得
        const imageData = refCanvas.current.toDataURL('image/png')
        setPhoto(imageData)  // 撮った写真を状態に保存
      }
    }
  }
  ...
  return (
    <div>
      <video ref={refVideo} autoPlay muted playsInline />
